...
- External provider runs may consume quota or generate costs. Report the exact external-call count
  and obtain explicit user authorization for that run before sending any provider request. Approval
  of implementation or run strategy is not authorization to send.
