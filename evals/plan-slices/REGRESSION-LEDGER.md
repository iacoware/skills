...
|---|---|---|---|---|---|---|---|---|---|---|
| R-001 | `2c89e7f` | `judgement` — `NOTES.md` § *Confine di scope vs identità* | reconstructed | The plan places identity after the differentiator. | reading | — | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | Split (2026-08-06): the seam claim became `R-012` | regressed on `CX` in CON-6 (2026-08-07) against `28b5460` — both instruments, identity at slice 2 before the differentiator at 3–4 |
| R-002 | `d977043` | `intersection-theme` — `REVIEW` CON-4 § *Sweep sistematico delle contraddizioni* ≡ *Explicit handling of source contradictions* | reconstructed and re-tuned | Every choice the plan declares open names the `NOW` slices it blocks, in whatever section it declares it. | validator-automatable on the open-choices section, whatever its title: every entry cites at least one existing `NOW` slice number | — | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | first member absorbed into `R-010` on 2026-08-06 | not falsified ×0 — the two CON-6 `verdetto` instruments disagree on `CC` (`holds` / `falsified`), so CON-6 does not count as a test; report CON-6 |
| R-003 | `d977043` | `intersection-theme` — `REVIEW` CON-4 § *Decisioni mai prese distinte dalle decisioni prese* ≡ *Explicit handling of undecided choices* | reconstructed and re-tuned | No `NOW` slice depends on an external choice — provider, model, service, or adapter — that is not made by a citable source, or made by the plan among the alternatives the brief declares acceptable, or declared open together with the slice it blocks, in whatever section it declares it; a qualifying adjective — `cheap`, `multilingual`, `managed` — does not count as a choice. | reading: the inventory of external dependencies requires comparison with the sources and with the brief's `Accepted alternatives` | — | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | — | not falsified ×1 |
| R-004 | `d977043` | `intersection-theme` — `REVIEW` CON-4 § *Ammissione in NOW subordinata a una domanda delle fonti* ≡ *Trace scope and horizon ownership* | reconstructed | No `NOW` slice delivers a behaviour the sources do not request. | reading — the skill places the tracing in reasoning, not in the plan | — | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | Split (2026-08-06): the `LATER` claim became `R-013`, the `OUT-OF-SCOPE` claim `R-014` | not falsified ×1 — the two CON-6 `verdetto` instruments disagree on both plans, so CON-6 does not count as a test; report CON-6 |
| R-005 | `d977043`, `9aa2586` | `intersection-theme` — `REVIEW` CON-4 § *Continuità del tema* ≡ *Keep a theme and its recovery path contiguous* | reconstructed | If a `NOW` slice names a failure mode in its own `Verification` and another `NOW` slice is its remedy, no slice of a different theme is placed between the two. | reading for the failure→remedy coupling; theme interposition is automatable on the slices' `*(Theme: X)*` annotation | — | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | — | not falsified ×2 |
| R-006 | `d977043`, `9aa2586` | `intersection-theme` — `REVIEW` CON-4 § *Pipeline o adapter condiviso con un solo proprietario* ≡ *Open shared pipelines and adapters once, after their producers* | reconstructed and re-tuned | A pipeline or adapter shared by several paths is opened in the `Includes` of a single `NOW` slice. | partly automatable — the same adapter named in the `Includes` of two slices is structural, recognizing that two names denote one adapter is reading | — | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | Split (2026-08-06): the reuse claim became `R-015`, the ordering claim `R-016`; the `9aa2586` component of `Commit` stays here unattributed, no claim resolves to it | not falsified ×1 — both CON-6 instruments read `holds` on both plans, but `recidiva` pairs `B#5` to this row; the conflict is unresolved, so CON-6 does not count as a test; report CON-6 |
| R-007 | `d977043` | `intersection-theme` — `REVIEW` CON-4 § *Audit esplicito di split* ≡ *Split capabilities and enablers with independent risks* | reconstructed and re-tuned | No `Enabler` slice validates uncertainties across more than one subsystem: its `Verification` cannot fail for causes that, in the brief's `Material uncertainties`, belong to different `Subsystem`s. Several entries of the same subsystem are one uncertainty, even when the answer invalidates the choice being verified. | reading: the per-pair split verdict lives in the unpublished ledger, only the outcome stays observable on the plan; the list of uncertainties, subsystems, and decisions that change is published by the brief | — | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | — | not falsified ×2 |
| R-008 | `9aa2586` | `intersection-theme` — `REVIEW` CON-4 § *Use repeatable, decision-changing verification*, part «each theme has a first validation» | reconstructed | Every theme's `First validation` points to a slice whose `Outcome` covers the theme's entire desired outcome. | validator for the existence of the reference; reading for outcome coverage | — | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | `Enabler` clause and its exception absorbed into `R-011` on 2026-08-06 | regressed on `CX` in CON-5 (2026-08-04) against `9aa2586` — rows C and D of that regression; row A left with the absorption. The two CON-6 instruments disagree on `CX`, so CON-6 adds nothing |
| R-009 | `a06a5cc` | `judgement` — commit message of `a06a5cc`, defect observed on a graded plan | reconstructed | No `Outcome` of a `NOW` slice preceding identity promises a real user: every slice that precedes identity and delivers a behaviour names its own audience, developer or tester on the declared non-public environment. | reading: the audience is read from the `Outcome`s, but deciding whether a slice delivers to an end user requires judgement | — | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | Split (2026-08-06): the deferral-threshold claim became `R-017` | not falsified ×1 — the two CON-6 `verdetto` instruments disagree on `CC`, so CON-6 does not count as a test; report CON-6 |
| R-010 | `87150d3` → `550077f` | `judgement` — correction of `R-002`, first member, regressed on `CC` in CON-5 | ex-ante (own limb); reconstructed (absorbed limb) | No `Includes` or `Verification` bullet asserts in non-conditional form one side of an unresolved choice. Unresolved covers a conflict between the sources — those listed under `Known conflicts` in the brief, plus those demonstrable by citing two sources in disagreement — and, in the slices that choice blocks, any other choice the plan does not resolve by citing a selecting source. Declaring the choice under `Open questions` or assigning it a spike does not resolve it. | reading: recognizing which slices an open choice blocks requires comparing the declaration with the bullets | The correction comes from a violation on one model only: `CX` violated neither member of `R-002` and already used conditional wording. That is the typical way `judgement` applies a false rule. The failure to watch for is not the return of assertive wording but its opposite: plans that defer everything to the pending decision and publish nothing verifiable any more. If it appears, the defect is in `R-010`, not in the plans. | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high; boundary: the clause was reformulated by this cycle's entry `A#1`/`B#1`, so the counter restarts against the pending text | `R-002` first member (2026-08-06) — that limb regressed on `CC` in CON-5 against `d977043` | not falsified ×0 — `holds` on both plans in CON-6 against `28b5460`, then re-anchored on 2026-08-07 to `550077f`, the CON-6 reformulation of its clause: ×k counts cycles against a text, and the text changed |
| R-011 | `eb926bb` | `judgement` — correction of `R-008`, `Enabler` clause, regressed on `CX` in CON-5 | ex-ante (own claim); reconstructed (absorbed clause) | Every row of the `Themes` table has its `First validation` resolve to a `NOW` slice not annotated `*(Enabler: …)*`, unless its `Desired outcome` cell carries the `*(Developer outcome)*` marker. | validator: the check crosses two facts the plan already publishes, the slice number resolved by the cell and the title tag of that slice | The marker is declarative: a plan can attach it to a desired outcome that is not a developer's. The validator cannot know, and that residue is `reading`. The failure to watch for is the marker attached to get past the check, not its absence. | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | `R-008`'s `Enabler` clause and its exception (2026-08-06) — that clause regressed on `CX` in CON-5 against `9aa2586`, row A | not falsified ×1 — first test of the merged wording |
| R-012 | `2c89e7f` | `judgement` — `NOTES.md` § *Confine di scope vs identità* | reconstructed | The plan declares under `Cross-functional concerns` the single seam from which the current scope resolves. | reading, partly automatable on the presence of the declaration under `Cross-functional concerns` | — | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | Split from `R-001` m2 (2026-08-06) | not falsified ×2 |
| R-013 | `d977043` | `intersection-theme` — `REVIEW` CON-4 § *Ammissione in NOW subordinata a una domanda delle fonti* ≡ *Trace scope and horizon ownership* | reconstructed | Every `LATER` entry states a `Promotion trigger`. | validator-automatable on the template's structure; not implemented — `validate_plan.py` checks only that the section carries a list | — | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | Split from `R-004` m2 (2026-08-06) | not falsified ×2 |
| R-014 | `d977043` | `intersection-theme` — `REVIEW` CON-4 § *Ammissione in NOW subordinata a una domanda delle fonti* ≡ *Trace scope and horizon ownership* | reconstructed | Every `OUT-OF-SCOPE` entry states an exclusion rationale. | validator-automatable on the template's structure; not implemented — `validate_plan.py` checks only that the section carries a list | — | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | Split from `R-004` m3 (2026-08-06) | not falsified ×2 |
| R-015 | `d977043` | `intersection-theme` — `REVIEW` CON-4 § *Pipeline o adapter condiviso con un solo proprietario* ≡ *Open shared pipelines and adapters once, after their producers* | reconstructed and re-tuned | A `NOW` slice that reuses a pipeline or adapter opened by an earlier slice declares it as reuse. | reading | The requirement was added to the parent row after CON-5 *because* `CX` never declared reuse, and `support/CLAUSE-ROW-MAP.md` records the anchor as `unresolved`: no clause of `SKILL.md` states it. A verdict against this row looks like a skill regression without being one until Fase 4 decides it. | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | Split from `R-006` m1, second half (2026-08-06) | regressed on `CC` and `CX` in CON-6 (2026-08-07) against `28b5460`, unanimously on both instruments — no clause of `SKILL.md` states the requirement, which is what its `Watch for` predicted; Fase 4 decides |
| R-016 | `d977043` | `intersection-theme` — `REVIEW` CON-4 § *Pipeline o adapter condiviso con un solo proprietario* ≡ *Open shared pipelines and adapters once, after their producers* | reconstructed and re-tuned | The `NOW` slice that opens a pipeline or adapter shared by several paths follows every `NOW` slice that feeds it input, except when it validates controlled inputs that traverse the production computation and the scenario's brief admits early validation. | reading for identifying the producers | The exception resolves to the scenario's brief, § `Accepted alternatives`, not to `SKILL.md` — see `support/CLAUSE-ROW-MAP.md` § *Anchors that resolve outside `SKILL.md`*. | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | Split from `R-006` m2 (2026-08-06) | regressed on `CX` in CON-6 (2026-08-07) against `28b5460` — the LLM extraction adapter opens at slice 7, before the paste path that feeds it at slice 8 |
| R-017 | `a06a5cc` | `judgement` — commit message of `a06a5cc`, defect observed on a graded plan | reconstructed | If more than two `NOW` slices deliver behaviour to an end user before identity, `Ordering criteria` justifies the residual deferral once, naming the evidence that requires it. | reading: the justification is read from `Ordering criteria`, but deciding whether a slice delivers to an end user requires judgement | — | 2026-08-07 | CON-6 · `CC`+`CX` · brief+plans+sources · gen `claude-opus-5` medium + `gpt-5.6-sol` medium · verdict CON-6 `verdetto`, two instruments — `CC` `claude-opus-5` high, `CX` `gpt-5.6-sol` high | Split from `R-009` m2 (2026-08-06) | not falsified ×2 |
| R-018 | `550077f` | `intersection-theme` — `REVIEW` CON-6, `A#1` ≡ `B#1`; both reviews classify the entry shared, the wording comes from `A` | ex-ante | Every behaviour that two sources describe incompatibly appears among the plan's open entries with the `NOW` slices it blocks, whether or not either source names a provider, model, service, or adapter. | reading: deciding that two sources describe one behaviour incompatibly requires comparing them, and the brief's `Known conflicts` lists only some of the cases the claim quantifies over | The failure to watch for is the opposite one: a plan that lists non-conflicts among its open entries to satisfy the rule, and defers slices that nothing in the sources leaves open. If it appears, the defect is in `R-018`, not in the plans. | — | not measured yet — first test at CON-7 | Born beside the re-anchoring of `R-010` (2026-08-07): `R-010`'s claim already covered the entry's own `Binary test`, this row carries the limb no active row predicted | to verify |

Active rows: **18**. Dormant: **0**.
...
**CON-6 took the count from 17 to 18, and the one row that entered is a real prediction.** `R-018`
comes from the single entry both CON-6 `REVIEW`s classify as shared. It is *not* the entry's own
`Binary test`: that test is entailed by `R-010`'s claim, which already reached conflicts
«demonstrable by citing two sources in disagreement», so writing it as a row would have counted one
piece of evidence twice — the error the two absorptions of 2026-08-06 closed. `R-018` carries the
limb of the applied change that no active row predicted: that the incompatible behaviour is
**listed** among the plan's open entries. `R-002` conditions on a choice already declared open and
`R-003` reaches only provider, model, service and adapter choices; neither covers it. The same entry
also re-anchored `R-010`, which is why `active rows 17 → 18` accuses one addition and not two.
