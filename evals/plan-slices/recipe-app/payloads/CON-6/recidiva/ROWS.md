# Ledger rows — every claim the ledger currently predicts

Each row is a prediction about a generated delivery plan: a defect that, once corrected, does not
come back. Pair every entry of the two reports with the row whose claim its defect would falsify,
or with `none`.

The claims are copied verbatim from the ledger, by id. Nothing else of the row travels with them:
no state, no counter, no history, no origin, and no indication of how long a row has stood.
...
| R-011 | Every row of the `Themes` table has its `First validation` resolve to a `NOW` slice not annotated `*(Enabler: …)*`, unless its `Desired outcome` cell carries the `*(Developer outcome)*` marker. |
...
| R-016 | The `NOW` slice that opens a pipeline or adapter shared by several paths follows every `NOW` slice that feeds it input, except when it validates controlled inputs that traverse the production computation and the scenario's brief admits early validation. |
