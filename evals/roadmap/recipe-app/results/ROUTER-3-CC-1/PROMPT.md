...
**Starting state:** `fixtures/redrawn/`

```bash
mkdir -p evals/roadmap/recipe-app/results/ROUTER-3-CC-1/.roadmap
cp -R evals/roadmap/recipe-app/fixtures/redrawn/. evals/roadmap/recipe-app/results/ROUTER-3-CC-1/.roadmap/
```
...
> Then, the request:
>
> Treat evals/roadmap/recipe-app/results/ROUTER-3-CC-1/ as the project root; the roadmap is in its .roadmap/,
...
>
> - The only repository files you may read are `evals/roadmap/recipe-app/results/ROUTER-3-CC-1/.roadmap/` and
...
The session asked what had been delivered, and attached a second question to it about who may
publish. Only the first was answered — see the note on R-005 in `../../EVALUATION-RULES.md`.
