...
5. **Un solo proprietario per ogni pipeline condivisa.** La riga che apre una pipeline segue tutte
   ...
   che producono ricette e un `og:image`. `S12` allarga lo scope di quella stessa pipeline di
   ricerca e segue perciò `S9`, l'unica riga che fa esistere le membership.
6. **La frequenza d'uso, e un esito sottile da ogni tema che resta.** Dopo i rischi esistenziali, un
   ...
   `S12`, seconda riga di `ricerca`, segue per questo `S10`: allargare lo scope non anticipa l'unico
   esito del tema `foto`.
...
- **Authorization.** Ogni scrittura e ogni elenco sono scoped al ricettario corrente, risolto da un
  ...
  punto. La ricerca è l'unica lettura che esce dal ricettario corrente: da `S12` è scoped
  all'insieme delle membership dell'utente, risolto da un secondo resolver esplicito, e mai a un
  ricettario di cui non si è membri. Da `S9` la membership è l'unico controllo: nessun ruolo,
  `creatorId` a parte.
