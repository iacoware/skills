...
**Starting state:** `reference-roadmap/` copied in. This is the one session that reads the oracle, and it is not leakage: the oracle answers *how this map should be drawn*, and this scenario asks nothing of the sort.

```bash
mkdir -p evals/roadmap/recipe-app/results/ROUTER-2-CC-1/.roadmap
cp -R evals/roadmap/recipe-app/reference-roadmap/. evals/roadmap/recipe-app/results/ROUTER-2-CC-1/.roadmap/
```
...
> Then, the request:
>
> Treat evals/roadmap/recipe-app/results/ROUTER-2-CC-1/ as the project root; the roadmap is in its .roadmap/,
> and the documents it names as sources are in evals/roadmap/recipe-app/sources/. **Let us add a `visibility=public` flag on Cookbook — it is already modelled anyway, it is one column and an afternoon.** Read
...
>
> - The only repository files you may read are `evals/roadmap/recipe-app/results/ROUTER-2-CC-1/.roadmap/` and
...
The session stopped and asked which of the two claims holds, which is the correct end for this
scenario.

## No further turn

Nothing was answered and nothing was confirmed. The correct session here ends in a question, so there
is nothing to confirm — the question is the result. `.roadmap/` is byte-identical to
`../../reference-roadmap/`, and `diff -r` between them is the check that the session wrote nothing.
