...
e il contatore degli id riparte da dove l'hanno lasciato. Da allora non è stato consegnato niente: il
registro è ancora quello della ridisegnata. È arrivata invece una dichiarazione sulla scala — migliaia
di ricette in ricettari che nessuno ha messo insieme — che era il presupposto non scritto di `S13` ed
è diventata una riga sua, `S17`.
...
3. **Il differenziatore, che ora si misura su un corpus e non su un ricettario.** Perde due volte, e
   solo due. Contro il n. 2, perché la ricerca su tutto il pubblico non è misurabile finché il
   pubblico non esiste, e nessun input controllato può sostituirlo senza aggirare proprio
   l'autorizzazione in prova. E contro `S17`, perché misurarla su un corpus messo insieme da noi
   risponde a una domanda diversa da quella posta: il corpus vero è fatto di ricettari che nessuno ha
   coordinato, e un banco di prova che ci siamo scelti la fa passare senza dire niente.
...
6. **Un esito sottile da ogni tema prima del secondo sullo stesso tema.** `S12`, `S13` e `S14`
   aprono i tre temi, e solo dopo `S15` torna sulla scoperta. `S17` sta fra `S12` e `S13` senza
   aprire niente: è un enabler, e il tema `scoperta` resta validato da `S13`. Questa mappa non deroga.
...
| `scoperta` | Cerchi con parole tue e trovi ricette in tutti i ricettari pubblici — migliaia, che nessuno ha messo insieme — anche scritte in un'altra lingua, senza account. | `S13` |
...
- `scoperta` — La scala non è più un'assunzione: l'autore ha dichiarato migliaia di ricette in
  ricettari che nessuno ha messo insieme, dentro il ≤10k delle sorgenti. Resta assunto che pgvector
  con HNSW regga quel corpus come regge il privato, e che la somiglianza semantica da sola basti a
  ordinare risultati che vengono da mani diverse. `S13` può smentire entrambe, e l'esclusione del
  vector DB dedicato sta in piedi sulla prima.

## Open questions

- `pubblico`, `trovabilita`, `S17` — Ripubblicazione del materiale estratto. La riga delle foto scarica
  ...
  senza immagine è un'anteprima povera. E adesso la stessa domanda si pone su migliaia di catture,
  non su una pagina: è ciò che `S17` va a fare.
- `goal` — Chi ha il diritto di pubblicare. Qualunque membro rende pubblico un ricettario, o la
  ...
  e la risposta decide se esiste una riga di curatela che questa mappa oggi non ha. È anche ciò che
  blocca `S17`.
...
| S17 | [Corpus pubblico alla scala dichiarata](slices/S17-corpus-pubblico.md) | scoperta | enabler | medium | needs-decision | mixed | S12 |
