...
`✗` rosso, `✓` verde con la prova nel `REVIEW.md`, `·` non registrata dal run.

| Violazione | m-1 | CC-2 | CC-3 | CC-4 | Commit che l'ha presa di mira |
|---|:--:|:--:|:--:|:--:|---|
| **R-015 / C1** — il lato preso solo in un bullet di riga | ✗ | ✗ | ✗ | ✗ | nessuno |
| **H5** — React Query assente | ✗ | ✗ | ✗ | ✗ | nessuno (e nemmeno l'oracolo lo nomina) |
| **R-009** — first validator che copre metà promessa | ✓ | ✗ | ✗ | ✗ | `779bf17`, **non testato** |
| **R-008** — theme compression | ✗ | ✗ | · | ✗ | `ff63c96` (metà verdetti), `779bf17` **non testato** |
| **R-022** — `Includes` che decide ciò che è indeciso | ✗ | ✗ | · | ✗ | nessuno |
| **R-017 published order** | ✗ | ✗ | ✓ | ✗ | `f77bc61` |
| **R-017 false edge** (arco sostituibile) | ✗ | ✗ | ✓ | ✓ | `f77bc61` — **fix dimostrato** |
| **R-017 dropped edge** | · | · | ✗ | ✗ | `2bf0a12` |
| **R-020** — learning target doppio / claim non osservato | ✗ | ✗ | · | · | nessuno |
| **R-012** — ampiezza prima di profondità, e ranking | ✓ | ✓ | · | ✗ | nessuno (ma `d805196` toglie ciò che reggeva i verdi) |
| **R-035** — chiusura oltre le quattro parti | · | ✓ | ✗ | ✗ | nessuno |
...
### 1.1 — `R-015` / `C1`: il conflitto risolto dentro una riga e riportato da nessuna parte
...
### 1.2 — `R-022`: `Includes` decide ciò che la riga dichiara indeciso
...
### 1.3 — `R-012`: l'ordine, dopo che è stata tolta la sezione che lo argomentava
...
### 2.1 — `R-017`, *published order*: `S1 → S0` e `S3 → S1` tornano pubblicati
...
**Come si misura:** metà *drawing*, scenario 0. Serve un run nuovo. Va scritto insieme al fix 3.1,
perché i due si spingono in direzioni opposte.

## 3. Fix che non hanno preso

Uno solo.

### 3.1 — `R-017`, *dropped edge*: `S11` porta `—` e la sua `Verification` chiama tre righe
...
**Il fix.** Stesso file e stessa sezione del 2.1 — `drawing-the-map.md` § *Hard dependencies* — in
...
information»). Se ne accorgerebbe **R-017 `⚠ opposite`** nella direzione opposta, e il lookup del 2.1
