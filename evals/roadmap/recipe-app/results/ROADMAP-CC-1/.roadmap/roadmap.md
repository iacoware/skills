...
**Current state:** Niente è stato consegnato. Repository vuoto, nessun ambiente, nessun account
aperto. Questa è la prima stesura della mappa e sta solo sulle quattro sorgenti.
...
|---|---|---|
| `ricettario` | Il ricettario corrente si consulta e si corregge: vedi l'elenco, apri una ricetta, la scrivi a mano o la modifichi nello stesso form. | `S3` |
...
- `S0` — Provider Postgres: le sorgenti scrivono "Neon o Supabase", ma la sezione hosting dà per
  scontato due volte "dati su Neon". Si legge **Neon**. Correggibile fino a `S1` a costo quasi nullo.
- `S1` — "~$0/mese" e "Fly non ha più un free tier vero" non stanno insieme. Si legge il target come
  *centesimi al mese* con `suspend` + scale-to-zero, e i ~$3/mese della macchina sempre calda come
  la spesa che l'autore accetta se il cold start desse fastidio.
...
|---|---|---|---|---|---|---|---|
| S0 | [Repository, CI verde e account aperti](slices/S0-repository-e-account.md) | — | enabler | small | ready | mixed | — |
| S1 | [Scheletro deployato su Fly con Postgres e migrazioni](slices/S1-scheletro-deployato.md) | — | release | medium | ready | mixed | S0 |
| S2 | [Quanto regge il cross-lingua un embedder multilingue nel budget](slices/S2-spike-embedder-multilingue.md) | ricerca | spike | small | ready | mixed | — |
| S3 | [Il ricettario corrente: elenco, dettaglio e un solo form](slices/S3-ricettario-corrente.md) | ricettario | product | medium | ready | agent | — |
| S4 | [Aggiunta da link con JSON-LD e avanzamento reale](slices/S4-aggiunta-da-link.md) | cattura | product | medium | ready | agent | S3 |
...
| S7 | [Ricerca semantica cross-lingua nel ricettario corrente](slices/S7-ricerca-semantica.md) | ricerca | product | medium | needs-decision | agent | S2 |
| S8 | [Accesso con Google e ricettario dell'utente](slices/S8-accesso-google.md) | accesso | product | medium | needs-decision | mixed | S3 |
...
| S10 | [Foto multiple e copertina su object storage](slices/S10-foto-e-copertina.md) | foto | product | medium | ready | mixed | S3, S4 |
